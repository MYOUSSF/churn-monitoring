...
try:
    ...
    st.error(
        ...
        "Run the following in your terminal, then restart Streamlit:\n\n"
        "```bash\npip install xgboost\n```"
    )
    st.stop()
...
# ── Palette — clean, accessible, light-background ─────────────────────────────
C_BLUE   = "#3B82F6"   # clear blue
C_TEAL   = "#0D9488"   # teal — stable / positive
C_AMBER  = "#D97706"   # amber — warnings
C_ROSE   = "#DC2626"   # rose-red — retrain / negative
C_VIOLET = "#7C3AED"   # violet — survival / extra series
C_SLATE  = "#64748B"   # slate — secondary / gridlines

# matplotlib — white background, dark readable text
plt.rcParams.update({
...
if n_retrain:
    ...
    st.markdown(
        ...
        'no retrain triggered across all cohorts.</div>',
        unsafe_allow_html=True,
    )
...
k5.metric("Optimal threshold",
          f"{test_m['optimal_threshold']:.2f}", "vs 0.50 default")
...
with tab_monitor:
    ...
    st.caption("Adjust PSI and AUROC thresholds in the sidebar — "
               "the table and charts update live.")
    ...
    with col_psi:
        ...
        ax.axhline(0.10, ls="--", color=C_AMBER, lw=1.4, alpha=0.85,
                   label="Warning (0.10)")
    ...
    with col_auroc:
        ...
        if df_a["retrain_triggered"].any():
            ax.fill_between(df_a["cohort"], auroc_threshold, df_a["auroc"],
                            ...
                            alpha=0.12, color=C_ROSE, label="Retrain zone")
...
with tab_business:
    ...
    st.caption(f"LTV=${ltv:,} · Offer=${offer_cost:,} · "
               f"{monthly_at_risk:,} customers/month — adjust in sidebar.")
    ...
    b4.metric("Left on table at 0.50",
              f"${opt.monthly_savings - at_half.monthly_savings:,.0f}",
              "extra/month vs default", delta_color="inverse")
    ...
    with col_sweep:
        ...
        ax1.axvline(opt.threshold, ls="--", color=C_TEAL, lw=2,
                    label=f"Optimal τ={opt.threshold:.2f}")
        ax1.axvline(0.5, ls=":", color=C_SLATE, lw=1.5,
    ...
    key_thresholds = sorted({opt.threshold, 0.25, 0.30, 0.40, 0.50})
    rows = []
    ...
    st.info(
        f"💡 Moving from **0.50** to the optimal **{opt.threshold:.2f}** saves "
        f"**${opt.monthly_savings - at_half.monthly_savings:,.0f}/month** extra "
        f"(**${(opt.monthly_savings - at_half.monthly_savings) * 12:,.0f}/year**)."
    )
...
with tab_model:
    ...
    with col_cal:
        ...
        for mdl, label, color, ls in [
            (data["model"],      f"XGB raw ({horizon}d)",        C_ROSE, "--"),
    ...
    rates    = [full_df[f"churn_{h}d"].mean()
                for h in horizons if f"churn_{h}d" in full_df]
    ...
    st.pyplot(fig, use_container_width=True)
    plt.close(fig)
    st.caption("Weibull shape k=1.5 → increasing hazard over time.")
...
with tab_features:
    ...
    with col_eda1:
        available = [f for f in ["mou", "revenue", "eqpdays", "custcare",
                                  "overage", "months", "revenue_per_mou",
                                  "custcare_rate"]
                     if f in full_df.columns]
        feat_choice = st.selectbox("Distribution by churn status", options=available)
